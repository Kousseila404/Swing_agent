...
| P2-2 | pytest écrit dans les données **prod** | `data/trade_journal.duckdb` et `data/sector_momentum.json` modifiés pendant la suite (au moins `test_api_readonly.py`, `test_auto_proposer_gates.py`) ; `agent.log` pollué (le gate pytest n'existe que pour `Tracker`) | fixture autouse qui redirige `DUCKDB_PATH`, `CSV_PATH`, `data/` vers `tmp_path` ; garde-fou CI : hash des fichiers data avant/après |
