...
def compute_indicators(df: pd.DataFrame, params: BacktestParams) -> pd.DataFrame:
    ...
    # pandas_ta retourne : ADX_14 (trend strength), DMP_14 (+DI), DMN_14 (-DI)
    try:
        adx_result = ta.adx(df["High"], df["Low"], df["Close"], length=params.atr_period)
        if adx_result is not None and not adx_result.empty:
            adx_col = [c for c in adx_result.columns if c.startswith("ADX_")]
            df["ADX"] = adx_result[adx_col[0]] if adx_col else np.nan
        else:
            df["ADX"] = np.nan
    except Exception:
        df["ADX"] = np.nan
    ...
    df["EMA200"] = ta.ema(df["Close"], length=params.ema_trend)
    ...
    if bb is not None and not bb.empty:
        ...
        df["BBU"]      = bb[upper_col[0]] if upper_col else np.nan
        df["BBU_prev"] = df["BBU"].shift(1) if "BBU" in df.columns else np.nan
        df["BBL"]      = bb[lower_col[0]] if lower_col else np.nan   # ← Nouveau V4
        df["BBL_prev"] = df["BBL"].shift(1) if "BBL" in df.columns else np.nan
    else:
    ...
    try:
        ...
        df["ATR22"]          = ta.atr(df["High"], df["Low"], df["Close"], length=chand_period)
